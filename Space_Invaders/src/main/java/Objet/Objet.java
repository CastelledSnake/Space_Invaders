...
public class Objet extends Polygon {
    ...
    //crée un tir d'alien
    public static void tir_alien(Group aliens, Group tirs_aliens, int numtir, String direction) {
    ...
    public static int[] depalien(Group aliens, int pos_gr_alien, int deplacement, String direction) {
        ...
        if ((pos_gr_alien == 0 && deplacement == -1) || (pos_gr_alien == 400 && deplacement == 1)) {
            ...
            for (int i = 0; i < n_alien; i++) {
                aliens.getChildren().get(i).setLayoutX(aliens.getChildren().get(i).getLayoutX() + 2d);
            }
            pos_gr_alien++;
        } else if (deplacement == -1) {
            for (int i = 0; i < n_alien; i++) {
    ...
    public static void Tir(Group tirs, String direction) {
        int n = tirs.getChildren().size();
        if (direction.equals("UP")) {
            for (int i = 0; i < n; i++) {
                tirs.getChildren().get(i).setLayoutY(tirs.getChildren().get(i).getLayoutY() - 2d);
            }
        } else if (direction.equals("DOWN")) {
            for (int i = 0; i < n; i++) {
                tirs.getChildren().get(i).setLayoutY(tirs.getChildren().get(i).getLayoutY() + 2d);
